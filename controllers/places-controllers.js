...
const createPlace = async (req, res, next) => {
    ...
    try {
        ...
        const error = new HttpError('Creating place failed, please try again - invalid user.', 500
        )
        return next(error)
    }

    if (!user) {
        const error = new HttpError('We could not find a user for the provided Id', 404)
        return next(error)
    }
    console.log(user)
    try {
        ...
        const error = new HttpError('Creating place failed, please try again - transaction issue.', 500)
        return next(error)
    }
...
const updatePlace = async (req, res, next) => {
    const errors = validationResult(req)
    if (!errors.isEmpty()) {
        return next(new HttpError('Cannot update place with invalid inputs, please check your inputs', 422))
    }
